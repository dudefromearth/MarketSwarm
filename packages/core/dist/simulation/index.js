 ...
 */
import { blackScholes, dteToYears } from '../pricing/index.js';
// ============================================================
// Default Values
// ============================================================
 ...
 */
export function calculateLegValue(leg, spot, volatility, riskFreeRate, daysRemaining) {
 ...
 */
export function calculateLegGreeks(leg, spot, volatility, riskFreeRate, daysRemaining) {
    const timeToExpiry = dteToYears(daysRemaining);
    const result = blackScholes({
        spot,
        strike: leg.strike,
 ...
 */
export function calculatePositionValue(legs, spot, volatility, riskFreeRate, daysRemaining) {
 ...
 */
export function calculatePositionGreeks(legs, spot, volatility, riskFreeRate, daysRemaining) {
    const initial = { delta: 0, gamma: 0, theta: 0, vega: 0, rho: 0 };
    return legs.reduce((total, leg) => {
 ...
 */
export function simulatePosition(position, params) {
    const { spot, volatility = DEFAULT_VOLATILITY, riskFreeRate = DEFAULT_RISK_FREE_RATE, priceRange = DEFAULT_PRICE_RANGE, numPoints = DEFAULT_NUM_POINTS, daysForward = 0, } = params;
    ...
    for (let i = 0; i < numPoints; i++) {
        ...
        const value = calculatePositionValue(position.legs, price, volatility, riskFreeRate, daysRemaining);
    ...
    const greeks = calculatePositionGreeks(position.legs, spot, volatility, riskFreeRate, daysRemaining);
    return {
        curve,
        maxProfit,
        maxLoss,
 ...
 */
export function simulateLegs(legs, entryDebit, dte, params) {
    const { spot, volatility = DEFAULT_VOLATILITY, riskFreeRate = DEFAULT_RISK_FREE_RATE, priceRange = DEFAULT_PRICE_RANGE, numPoints = DEFAULT_NUM_POINTS, daysForward = 0, } = params;
    // Calculate price range
    const minPrice = spot * (1 - priceRange);
    const maxPrice = spot * (1 + priceRange);
    ...
    for (let i = 0; i < numPoints; i++) {
        const price = minPrice + i * priceStep;
        const value = calculatePositionValue(legs, price, volatility, riskFreeRate, daysRemaining);
        const pnl = value + entryValue;
        curve.push({ price, pnl });
        if (pnl > maxProfit)
            maxProfit = pnl;
    ...
    // Calculate current Greeks
    const greeks = calculatePositionGreeks(legs, spot, volatility, riskFreeRate, daysRemaining);
    return {
        curve,
        maxProfit,
        maxLoss,
