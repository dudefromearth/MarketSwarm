 ...
 */
export function recognizePositionType(legs) {
    const n = legs.length;
    if (n === 0) {
    ...
    const getStrike = (idx) => sorted[idx]?.strike ?? 0;
    const getQty = (idx) => sorted[idx]?.quantity ?? 0;
    // Singles
    if (n === 1) {
        return {
            type: 'single',
    ...
    if (n === 2) {
        ...
        if (!allSameExp && allSameRight) {
            if (s0 === s1) {
                ...
                const byExp = [...legs].sort((a, b) => new Date(a.expiration).getTime() - new Date(b.expiration).getTime());
    ...
    if (n === 4 && allSameExp) {
        ...
        if (allSameRight && allQtyOne) {
            const isCondor = (q0 * q3 > 0 && // Outer wings same sign
 ...
 */
export function strategyToLegs(strategy, side, strike, width, expiration) {
    switch (strategy) {
        case 'single':
            return [{
                    strike,
 ...
 */
export function getCenterStrike(legs) {
    if (legs.length === 0)
        return 0;
    const sorted = [...legs].sort((a, b) => a.strike - b.strike);
 ...
 */
export function getPrimaryExpiration(legs) {
    if (legs.length === 0)
        return '';
 ...
 */
export function getDominantSide(legs) {
 ...
 */
export function hasSameExpiration(legs) {
    if (legs.length <= 1)
        return true;
    const first = legs[0];
    if (!first)
 ...
 */
export function hasSameRight(legs) {
    if (legs.length <= 1)
        return true;
    const first = legs[0];
    if (!first)
