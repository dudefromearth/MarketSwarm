...
###############################################
# MarketSwarm – healer (Service Healer)
...
export DEBUG_HEALER="${DEBUG_HEALER:-false}"
...
SERVICE="healer"
MAIN="$ROOT/services/healer/main.py"
...
run_foreground() {
  ...
  echo "MAIN:        $MAIN"
  echo "DEBUG_HEALER:$DEBUG_HEALER"
  line
  echo ""
...
menu() {
  while true; do
    clear
    line
    echo " MarketSwarm – healer (Service Healer)"
    line
    echo ""
    ...
    echo "Select Option:"
    echo ""
    echo "  1) Run healer (foreground)"
    echo "  2) Toggle DEBUG_HEALER"
    echo "  3) Quit"
    echo ""
    line
    ...
    case "$CH" in
      ...
      2)
        if [[ "$DEBUG_HEALER" == "true" ]]; then
          DEBUG_HEALER="false"
        else
          DEBUG_HEALER="true"
        fi
        export DEBUG_HEALER
        echo "DEBUG_HEALER is now: $DEBUG_HEALER"
        sleep 1
        ;;
