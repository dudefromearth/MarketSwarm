    ...
    try:
        async with async_playwright() as p:
            ...
            if not resp or resp.status >= 400:
                log("article_raw", "warn", "⚠️",
                    f"Browser HTTP {resp.status if resp else '??'} for {url}")
                await browser.close()
                return ""
...
async def fetch_and_store_article(uid, url, title, category, r_intel):
    ...
    if await r_intel.exists(canon_key):
        return
    ...
    # 1. Store raw snapshot (debug & audit)
    ...
    # ------------------------------------------------------------
    # 2. Build canonical Tier-0 article (HTML STRIPPED HARD)
    # ------------------------------------------------------------
    canonical = build_canonical_article({
        "uid": uid,
        "url": url,
        "title": title,
        "category": category,
    ...
    # 3. Store canonical substrate to Redis
    await r_intel.hset(canon_key, mapping=as_mapping(canonical))
    await r_intel.zadd("rss:article_canonical:index", {uid: time.time()})

    log("article_schema", "ok", "🧱",
        f"Stored canonical substrate {uid[:8]} → rss:article_canonical")