    ...
    def upsert_afi_score(
        ...
        try:
            ...
            cursor.execute("""
                ...
                AS new_row
                ON DUPLICATE KEY UPDATE
                    afi_score = new_row.afi_score,
                    afi_raw = new_row.afi_raw,
                    wss = new_row.wss,
                    comp_r_slope = new_row.comp_r_slope,
                    comp_sharpe = new_row.comp_sharpe,
                    comp_ltc = new_row.comp_ltc,
                    comp_dd_containment = new_row.comp_dd_containment,
                    robustness = new_row.robustness,
                    trend = new_row.trend,
                    is_provisional = new_row.is_provisional,
                    trade_count = new_row.trade_count,
                    active_days = new_row.active_days,
                    calculated_at = new_row.calculated_at,
                    wss_history = new_row.wss_history,
                    afi_version = new_row.afi_version,
                    cps = new_row.cps,
                    repeatability = new_row.repeatability,
                    capital_status = new_row.capital_status,
                    leaderboard_eligible = new_row.leaderboard_eligible,
                    afi_m = new_row.afi_m,
                    afi_r = new_row.afi_r,
                    composite = new_row.composite,
                    comp_daily_sharpe = new_row.comp_daily_sharpe,
                    comp_drawdown_resilience = new_row.comp_drawdown_resilience,
                    comp_payoff_asymmetry = new_row.comp_payoff_asymmetry,
                    comp_recovery_velocity = new_row.comp_recovery_velocity,
                    confidence = new_row.confidence,
                    raw_afi_m = new_row.raw_afi_m,
                    raw_afi_r = new_row.raw_afi_r,
                    raw_sharpe_lifetime = new_row.raw_sharpe_lifetime
            """, (
                user_id, afi_score, afi_raw, wss,
                comp_r_slope, comp_sharpe, comp_ltc, comp_dd_containment,
